...
class ContributionUpdateService : Service() {

    private var serviceJob = Job()
    private val coroutineScope = CoroutineScope(serviceJob + Dispatchers.Main)

    companion object {
        const val UPDATE_CALCULATION = "updateCalculation"
...
//    }
    ...
    fun getFollowersList(notifications: OperationLog) {
        coroutineScope.launch {
            ...
            when (
                val result =
                    WeShareApplication.instance.repository.getUserInfo(UserManager.weShareUser!!.uid)
            ) {
                is Result.Success -> {
                    _error.value = null
